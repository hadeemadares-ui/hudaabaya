...
async function getOrdersFromKV(env) {
  if (env?.HUDA_KV) {
    try {
...
__name(getOrdersFromKV, "getOrdersFromKV");
async function saveOrdersToKV(env, orders) {
  memoryOrders = orders;
  if (env?.HUDA_KV) {
    try {
...
__name(saveOrdersToKV, "saveOrdersToKV");
async function onRequestGet(context) {
  ...
  return new Response(
    JSON.stringify({
      success: true,
      orders,
      serverTimeUTC: (/* @__PURE__ */ new Date()).toISOString()
    }),
    { headers: corsHeaders }
  );
}
...
async function onRequestPost(context) {
  try {
    ...
    if (action === "sync_all_orders" && Array.isArray(orders)) {
      ...
      globalOrders = [body, ...globalOrders.filter((o) => o.id !== body.id)];
    } else if (action === "update_status" && orderId) {
      globalOrders = globalOrders.map(
        (o) => o.id === orderId ? { ...o, orderStatus, trackingNumber: body.trackingNumber || o.trackingNumber, courier: body.courier || o.courier } : o
      );
    } else if (action === "update_payment" && orderId) {
      globalOrders = globalOrders.map((o) => o.id === orderId ? { ...o, paymentStatus } : o);
...
async function getProductsFromKVOrMemory(env) {
  if (env?.HUDA_KV) {
    try {
...
__name(getProductsFromKVOrMemory, "getProductsFromKVOrMemory");
async function saveProductsToKV(env, prods, delIds) {
  memoryProducts = prods;
  memoryDeletedIds = delIds;
  if (env?.HUDA_KV) {
    try {
...
__name(saveProductsToKV, "saveProductsToKV");
  ...
  try {
    ...
    if (action === "sync_all_products" && Array.isArray(products)) {
      ...
      if (Array.isArray(deletedIds)) {
        globalDeletedIds = Array.from(/* @__PURE__ */ new Set([...globalDeletedIds, ...deletedIds]));
      }
    } else if ((action === "add_product" || action === "update_product") && product) {
      globalProducts = [product, ...globalProducts.filter((p) => p.id !== product.id)];
...
__name(getSettingsFromKV, "getSettingsFromKV");
async function saveSettingsToKV(env, settings) {
...
__name(saveSettingsToKV, "saveSettingsToKV");
...
// ../.wrangler/tmp/pages-X4m9XE/functionsRoutes-0.08211740026281666.mjs
var routes = [
  {
