...
import { Star, ShieldCheck, Sparkles, SlidersHorizontal, PackageSearch, HeartHandshake, TrendingUp } from 'lucide-react';
...
export default function HomePage() {
  ...
  return (
    <div className="space-y-0">
      ...
      {/* Floating Green Sales & Profit Report Shortcut for Admin */}
      {isAdminAuthenticated && (
        <button
          onClick={() => {
            ...
            const shopContext = (window as any).__HUDA_TOGGLE_ADMIN__;
            if (shopContext) shopContext();
            else {
              const navbarAdminBtn = document.querySelector('header button[title*="หลังบ้าน"]') as HTMLButtonElement;
              if (navbarAdminBtn) navbarAdminBtn.click();
            }
          }}
          className="fixed bottom-6 right-6 z-40 bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-500 hover:from-emerald-500 hover:to-teal-400 text-white font-serif font-extrabold px-5 py-3 rounded-full shadow-2xl border-2 border-emerald-300 flex items-center gap-2 hover:scale-110 transition duration-300 group cursor-pointer"
          title="คลิกเพื่อดูรายงานยอดขาย & กำไรสุทธิ"
        >
          <TrendingUp className="w-5 h-5 text-emerald-200 animate-pulse group-hover:scale-125 transition" />
          <span className="text-xs sm:text-sm font-sans font-bold">📊 รายงานยอดขาย & กำไร (Financial Analytics)</span>
        </button>
      )}
